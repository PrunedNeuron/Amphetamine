...
object SendIconRequest {
    private const val BASE_URL = "https://api.ayushm.dev"
    ...
    private val service by lazy {
        Retrofit.Builder()
            ...
            .build().create(LithiumService::class.java)
    }
    ...
    private suspend fun buildTextFiles(
        ...
        if (uploadToArctic) jsonSb = StringBuilder("{\n\t\"icons\": [") // TODO: Should this be "components" instead?
        ...
        for ((iconName, app) in correctList) {
            ...
            if (jsonSb != null) {
                ...
                jsonSb.append("\n\t\t{\n")
                    ...
                    .append("\t\t\t\"component\": \"${app.component}\",\n")
                    .append("\t\t\t\"url\": \"https://play.google.com/store/apps/details?id=${app.packageName}\"")
                    .append("\n\t\t}")
            }

            Log.d("Request JSON", jsonSb.toString()) // TODO: Remove this in production
    ...
    private suspend fun uploadToArctic(
        ...
        return withContext(IO) {
            ...
            val response: LithiumResponse? = try {
                service.uploadRequest(apiKey, jsonContent)
            } catch (e: Exception) {
                null
            }

            // TODO: Remove the following if else blocks in production
            if (response != null) {
                Log.d("AMPHETAMINE RESPONSE: ", "status = " + response.status + "\tmessage = " + response.message)
            } else {
                Log.d("AMPHETAMINE RESPONSE: ", "was null. response = $response\n")
            }

            val success = response?.status?.contains("success", true) ?: false
//            val errorMessage = response?.error.orEmpty()
            val errorMessage = if (success) "" else "Sorry, the Lithium server did not respond or could not handle the request."
...
//                val uploadToArctic = apiKey.hasContent() // TODO: Fix this.
                val uploadToArctic = true
